...
static void command_help(void) {
    terminal_write("help clear pwd ls cd mkdir touch cat write rm meminfo desktop\n");
    terminal_write("write <file> <text>  - text file create or modify\n");
}

static void list_visitor(fs_node_t* node, void* context) {
    uint16_t* count = (uint16_t*)context;
    terminal_write(fs_type_name(node->type));
    terminal_write("  ");
...
static void execute(char* input) {
    ...
    if (strings_equal(command, "help")) command_help();
        ...
        else terminal_write("cd: nincs ilyen konyvtar\n");
        ...
        if (argument == 0 || fs_create(working_directory, argument,
            ...
            terminal_write("nem hozhato letre\n");
        ...
        else terminal_write("cat: nincs ilyen fajl\n");
        ...
        if (node != 0 && content != 0 && fs_write(node, content)) terminal_write("mentve\n");
        else terminal_write("write: fajl es szoveg szukseges\n");
    } else if (strings_equal(command, "rm")) {
        argument = next_word(&cursor); node = argument ? resolve(argument) : 0;
        if (!fs_remove(node)) terminal_write("rm: nem torolheto\n");
    } else if (strings_equal(command, "desktop")) {
        gui_show_desktop();
    } else if (strings_equal(command, "meminfo")) {
        terminal_write("RAM fajlrendszer: 32 bejegyzes, fajlonkent 255 byte\n");
    } else terminal_write("ismeretlen parancs: irj help-et\n");
}
...
void shell_init(void) {
    ...
    line_length = 0;
    terminal_write("DuoOS shell - RAMFS keszen. help\n");
    print_prompt();
}
